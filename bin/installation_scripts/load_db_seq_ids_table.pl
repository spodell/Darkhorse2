...
# creates, populates a Mysql table of information on sequences in Genbank nr
# expects input as the tab-delimited output of script collect_nr_species.pl, in
# the following format:

# full_id_num  ncbi_tax_id  species_name  num_amino_acids    
...
# compose database population command 
	...
	# number of fields in NCBI refseq catalog file should be 7  
		...
		my $annotation = "$fields[3]";
		my $seq_length = "$fields[4]";	

# get rid of inconvenient punctuation that chokes MySQL (' and /)
		$species_name =~ s/\'//g;
		$species_name =~ s/\\//g;
				
		$sql .= "('$full_id','$ncbi_tax_id','$species_name','$annotation','$seq_length'),\n";
